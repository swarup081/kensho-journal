...
// ... (Your other components like defaultAvatars, TabButton, etc. remain here unchanged)
...
const InsightCard = ({ icon: Icon, label, value, color }) => ( <div className="bg-gradient-to-br from-gray-900/80 to-black/30 p-6 rounded-xl shadow-lg border border-gray-800/50 flex items-center gap-5"> <div className={`p-3 rounded-full bg-${color}-500/10`}><Icon className={`h-7 w-7 text-${color}-400`} /></div> <div> <p className="text-sm text-gray-400">{label}</p> <p className="text-2xl font-bold text-white">{value}</p> </div> </div> );
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (user) {
        ...
        let { data: profileData } = await supabase.from('users').select(`name, avatar_url, email_notifications_enabled`).eq('id', user.id).single();
  ...
  const calculateStreaks = (entries) => { /* ... (your streak logic) ... */ return { currentStreak: 0, longestStreak: 0 }; };
  const updateAvatarUrlInDb = async (avatarUrl) => { if (!user) return; const { error } = await supabase.from('users').update({ avatar_url: avatarUrl }).eq('id', user.id); if (!error) { setProfile(prev => ({ ...prev, avatar_url: avatarUrl })); } };
  const handleFileUpload = async (event) => { if (!event.target.files || event.target.files.length === 0 || !user) return; const file = event.target.files[0]; const maxFileSize = 2 * 1024 * 1024; if (file.size > maxFileSize) { alert('File is too large. Please select an image smaller than 2MB.'); return; } if (profile.avatar_url && profile.avatar_url.includes(supabase.storage.url)) { const oldFilePath = profile.avatar_url.split('/avatars/')[1]; if (oldFilePath) { await supabase.storage.from('avatars').remove([oldFilePath]); } } const fileExt = file.name.split('.').pop(); const filePath = `${user.id}/${Date.now()}.${fileExt}`; setIsUploading(true); const { error: uploadError } = await supabase.storage.from('avatars').upload(filePath, file); if (!uploadError) { const { data } = supabase.storage.from('avatars').getPublicUrl(filePath); if (data) { await updateAvatarUrlInDb(data.publicUrl); } } setIsUploading(false); setIsModalOpen(false); };
  const handleRemovePhoto = async () => { if (!user || !profile.avatar_url) return; if (!profile.avatar_url.includes(supabase.storage.url)) return; const filePath = profile.avatar_url.split('/avatars/')[1]; const { error } = await supabase.storage.from('avatars').remove([filePath]); if (!error) { const placeholder = `https://placehold.co/128x128/A78BFA/FFFFFF/png?text=${profile.name?.[0] || 'K'}`; await updateAvatarUrlInDb(placeholder); } setIsModalOpen(false); };
  const handleAvatarSelect = async (avatarUrl) => { await updateAvatarUrlInDb(avatarUrl); setIsModalOpen(false); };
  const handleNameChange = async (e) => { e.preventDefault(); const trimmedName = newName.trim(); if (!user || !trimmedName) return; setProfile(prev => ({ ...prev, name: trimmedName })); setIsEditingName(false); await updateProfileName(trimmedName); };
  ...
  const handlePasswordReset = async () => {
    ...
    if (error) {
        setPasswordResetMessage('Error sending reset link.');
  ...
  const handleNotificationToggle = async (isEnabled) => {
    ...
    await supabase.from('users').update({ email_notifications_enabled: isEnabled }).eq('id', user.id);
  };
            ...
            <div className="mt-8">
              <AnimatePresence mode="wait">
                <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
                  {activeTab === 'profile' && (
                    <div>
                      <div className="bg-black/10 rounded-2xl shadow-2xl p-8"> <div className="flex flex-col sm:flex-row items-center gap-8"> <button onClick={() => setIsModalOpen(true)} className="relative group flex-shrink-0"> <img src={profile.avatar_url} alt="User Avatar" className="h-32 w-32 rounded-full ring-4 ring-purple-500/30" /> <div className="absolute inset-0 flex items-center justify-center bg-black/60 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200"> <Edit3 className="text-white h-8 w-8"/> </div> </button> <div className="flex-grow text-center sm:text-left"> {isEditingName ? ( <form onSubmit={handleNameChange} className="flex items-center gap-2"> <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} className="text-3xl font-bold text-white bg-gray-800 rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-purple-500" autoFocus /> <button type="submit" className="font-semibold bg-green-600 hover:bg-green-700 text-white py-1.5 px-4 rounded-lg transition-colors">Save</button> <button type="button" onClick={() => setIsEditingName(false)} className="font-semibold bg-gray-700/80 hover:bg-gray-700 text-white py-1.5 px-4 rounded-lg transition-colors">Cancel</button> </form> ) : ( <div className="flex items-center gap-3"> <h2 className="text-3xl font-bold text-white">{profile.name}</h2> <button onClick={() => setIsEditingName(true)} className="text-gray-400 hover:text-white"><Edit3 size={18} /></button> </div> )} <p className="text-gray-400 mt-1">{user?.email}</p> <p className="text-xs text-gray-500 mt-2">Member since {memberSince}</p> </div> </div> </div>
                      <div className="mt-8"> <h3 className="text-xl font-bold text-white mb-4" style={{ fontFamily: "'Lora', serif" }}>Your Journaling Insights</h3> <div className="grid grid-cols-1 sm:grid-cols-3 gap-6"> <InsightCard icon={BookOpen} label="Total Entries" value={stats.totalEntries} color="purple" /> <InsightCard icon={Repeat} label="Current Streak" value={`${stats.currentStreak} days`} color="orange" /> <InsightCard icon={Zap} label="Longest Streak" value={`${stats.longestStreak} days`} color="green" /> </div> </div>
                    </div>
                  )}
                  {activeTab === 'settings' && (
                      ...
                      <div className="divide-y divide-gray-700/50">
                          <div className="py-6 flex items-center justify-between">
                              <div>
                                  ...
                                  {/* --- THE FIX: Themed and styled message --- */}
                                  {passwordResetMessage && (
                                    <p className={`text-sm mt-2 font-semibold ${passwordResetError ? 'text-red-400' : 'text-purple-400'}`}>
                                      {passwordResetMessage}
                                    </p>
                                  )}
                              </div>
